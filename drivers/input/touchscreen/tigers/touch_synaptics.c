...
#define DOZE_WAKE_THRESH_REG 				(ts->common_fc.dsc.control_base+3)		/*                      */
#define DOZE_HOLDOFF_REG 				(ts->common_fc.dsc.control_base+4)		/*                   */
#define DOZE_RECAL_INTERVAL_REG 				(ts->common_fc.dsc.control_base+5)		/*                         */
...
#define ANALOG_REPORT_RATE_LSB_REG			0x06	/*          */
#define ANALOG_REPORT_RATE_MSB_REG			0x07	/*          */
#define ANALOG_CURRENT_NOISE_STATE_REG		0x08	/*          */
#define ANALOG_CID_IM_REG					0x09	/*          */
#define ANALOG_FREQ_SCAN_IM_REG				0x0a	/*          */
#define ANALOG_SENSE_FREQ_SEL_REG			0x0b	/*          */
...
int set_fw_info(struct synaptics_ts_data *ts, struct touch_fw_info *fw_info)
{
	...
	case TOUCH_CHIP_REV_B:
			...
			case TOUCH_PANEL_BAR_PATTERN:
					if (get_touch_panel_info(ts) == TOUCH_PANEL_H_PATTERN) { /*                 */
			...
			case TOUCH_PANEL_H_PATTERN:
					...
					if (lge_get_board_revno() >= HW_REV_H) /*                    */
						memcpy(&SynaFirmware,&SynaFirmware_d,sizeof(SynaFirmware));
					else
						memcpy(&SynaFirmware,&SynaFirmware_c,sizeof(SynaFirmware));
