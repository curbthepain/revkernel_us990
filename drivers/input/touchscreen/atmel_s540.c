...
#define MXT_PATCH_BAT_MODE_EVENT	0	/*              */
#define MXT_PATCH_TA_MODE_EVENT	1	/*            */
#define MXT_PATCH_KNOCKON_BAT_MODE_EVENT	2	/*                      */
#define MXT_PATCH_WAKEUP_BAT_MODE_EVENT	3	/*                      */
#define MXT_PATCH_KNOCKON_TA_MODE_EVENT	4	/*                     */
#define MXT_PATCH_WAKEUP_TA_MODE_EVENT	5	/*                    */
...
#define MFTS_T6_ADDRESS			355 /*         */
#define ORIGINAL_T6_ADDRESS		367	/*         */
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	if (status & MXT_T100_DETECT) {
		...
		if(data->t100_aux_area){
			...
			}else if(area >= data->T100_palm_threshold){ /*      */
...
static void mxt_proc_t35_messages(struct mxt_data *data, u8 *message)
{
	...
	if(data->mxt_character_enable && msg == 0x43){	/* */
		dev_info(dev, "character C recognize.\n");
	}else if(data->mxt_character_enable && msg == 0x4D){	/* */
		dev_info(dev, "character M recognize.\n");
	}else if(data->mxt_character_enable && msg == 0x57){	/* */
		dev_info(dev, "character W recognize.\n");
	}else if(data->mxt_volume_enable && msg == 0x83){	/*      */
		dev_info(dev, "Volume Up recognize.\n");
	}else if(data->mxt_volume_enable && msg == 0x84){	/*        */
	...
	if(msg == 0x04) {
		if(quick_cover_status == 1){
			...
			dev_dbg(dev, "QuickCoverSize X %d-%d  Y %d-%d \n",
				...
				data->qwindow_size->y_min, data->qwindow_size->y_max);	/*                                    */

			if(!((x > data->qwindow_size->x_min ) && (x < data->qwindow_size->x_max)
				&& (y > data->qwindow_size->y_min) && (y < data->qwindow_size->y_max)))	//                         
#else
  ...
  mutex_lock(&mxt_early_mutex);
	...
	for (i = 0; i < data->pdata->numtouch; i++) {
		...
		dev_dbg(dev, "quick_cover_status %d  use_quick_window %d hall ic %d\n",
			...
			if (quick_cover_status || (use_quick_window && (cradle_smart_cover_status() == 1))){	/*                                        */
...
static int mxt_set_t7_power_cfg(struct mxt_data *data, u8 sleep)
{
	...
	struct t7_config knockon = { .idle = 32, .active = 15 };	/*                        */
...
static ssize_t mxt_object_control(struct mxt_data *data, const char *buf, size_t count)
{
	...
	}else if (!strncmp(command, "write", 4)){	/*     */
...
static void change_ime_drumming_func(struct work_struct *work_ime_drumming)
{
	...
	if (ime_drumming_status && !data->suspended){
		...
		enable = 35;
		if(touch_test_dev->charging_mode){ //       
			jump_limit = 15;
		} else {
			jump_limit = 45;
		}
...
err_t atmel_ts_lpwg(struct i2c_client* client, u32 code, u32 value, struct point *tci_point)
{
	...
	case LPWG_READ:
		if (data->mxt_password_enable) {
			if( (g_tci_press[0].x == -1) && (g_tci_press[0].x == -1))	//                            
			{
