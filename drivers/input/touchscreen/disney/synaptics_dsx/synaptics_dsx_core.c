...
static ssize_t synaptics_rmi4_f01_flashprog_show(struct device *dev,
		struct device_attribute *attr, char *buf);
#ifndef CUST_LGE_TOUCH_BRING_UP
...
static int synaptics_rmi4_check_status(struct synaptics_rmi4_data *rmi4_data, bool *was_in_bl_mode);
#endif
...
int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK && rmi4_data && rmi4_data->pdev) {
		blank = evdata->data;
...
#ifdef CUST_LGE_TOUCH_BRING_UP
static const struct platform_device_id platform_rmi4_id_table[] = {
...
#ifdef CUST_LGE_TOUCH_BRING_UP
	.id_table = platform_rmi4_id_table,
#endif
};
