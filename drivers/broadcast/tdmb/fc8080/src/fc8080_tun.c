...
static fci_s32 fc8080_read(HANDLE handle, fci_u8 addr, fci_u8 *data)
{
	fci_s32 res;
...
fci_s32 fc8080_tuner_init(HANDLE handle, fci_u32 band)
{
	fci_s32 i;
	fci_s32 n_rfagc_pd1_avg, n_rfagc_pd2_avg;

	fci_u8 rfagc_pd2[6], rfagc_pd2_avg, rfagc_pd2_max, rfagc_pd2_min;
	fci_u8 rfagc_pd1[6], rfagc_pd1_avg, rfagc_pd1_max, rfagc_pd1_min;
	...
	rfagc_pd1_avg = (fci_u8) n_rfagc_pd1_avg;
	...
	rfagc_pd2_avg = (fci_u8) n_rfagc_pd2_avg;
...
fci_s32 fc8080_set_freq(HANDLE handle, fci_u32 band, fci_u32 freq)
{
	fci_u32 f_diff, f_diff_shifted, n_val, k_val;
	fci_u32 f_vco, f_comp;
	fci_u8 div_ratio, lo_mode, r_val, data_0x55;
	...
	data_0x55 = ((r_val == 1) ? 0x00 : 0x10) + (fci_u8) (k_val >> 16);

	fc8080_write(handle, 0x51, 7 + 8 * lo_mode);
	fc8080_write(handle, 0x55, data_0x55);
	fc8080_write(handle, 0x56, (fci_u8) ((k_val >> 8) & 0xff));
	fc8080_write(handle, 0x57, (fci_u8) (((k_val) & 0xff)));
...
fci_s32 fc8080_get_rssi(HANDLE handle, fci_s32 *rssi)
{
	fci_s32 res = BBM_OK;
	fci_s32 pga = 0;

	fci_u8 lna, rfvga, filter, preamp_pga, a1, ext_lna , a2 , a3, k, crntmode1,
		crntmode0;
	fci_u8 temp = 0;
...
fci_s32 fc8080_tuner_deinit(HANDLE handle)
{
	return BBM_OK;
}
