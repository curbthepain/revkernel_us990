...
int tunerbb_drv_fc8300_set_channel(s32 f_rf, u16 mode, u8 subch)
{
	...
	print_log(NULL, "FC8300 tunerbb_drv_fc8300_set_channel %d, %d\n"
		, f_rf, mode, subch);
	if (mode)
...
int tunerbb_drv_fc8300_get_oneseg_antenna(int ant
	, int ber, int cn, s32 brd_type)
{
	u32 antlvl;
...
int tunerbb_drv_fc8300_Get_SignalInfo(struct fc8300Status_t *st, s32 brd_type)
{
	...
	res = bbm_com_bulk_read(NULL, DIV_MASTER
		, BBM_DM_DATA, (u8 *) &dm, sizeof(dm));
	...
	if (dm.vit_a_ber_rxd_rsps) {
		if (dm.vit_a_ber_err_rsps > 429496)
			st->per_a = ((u32)dm.vit_a_ber_err_rsps * 100
			/ ((u32) dm.vit_a_ber_rxd_rsps / 100));
		else
			st->per_a = ((u32)dm.vit_a_ber_err_rsps * 10000
			/ (u32) dm.vit_a_ber_rxd_rsps);
	} else
		st->per_a = 10000;

	if (dm.vit_b_ber_rxd_rsps) {
		if (dm.vit_b_ber_err_rsps > 429496)
			st->per_b = ((u32) dm.vit_b_ber_err_rsps * 100
			/ ((u32) dm.vit_b_ber_rxd_rsps / 100));
		else
			st->per_b = ((u32) dm.vit_b_ber_err_rsps * 10000
			/ (u32) dm.vit_b_ber_rxd_rsps);
	} else
		st->per_b = 10000;

	if (dm.vit_c_ber_rxd_rsps) {
		if (dm.vit_c_ber_err_rsps > 429496)
			st->per_c = ((u32) dm.vit_c_ber_err_rsps * 100
			/ ((u32) dm.vit_c_ber_rxd_rsps / 100));
		else
			st->per_c = ((u32) dm.vit_c_ber_err_rsps * 10000
			/ (u32) dm.vit_c_ber_rxd_rsps);
	} else
		st->per_c = 10000;

	if (dm.dmp_a_ber_rxd_bits) {
		if (dm.dmp_a_ber_err_bits > 429496)
			st->ber_a = ((u32) dm.dmp_a_ber_err_bits * 100
			/ ((u32) dm.dmp_a_ber_rxd_bits / 100));
		else
			st->ber_a = ((u32) dm.dmp_a_ber_err_bits * 10000
			/ (u32) dm.dmp_a_ber_rxd_bits);
	} else {
		st->ber_a = 10000;
	}
	...
	if (dm.dmp_b_ber_rxd_bits) {
		if (dm.dmp_b_ber_err_bits > 429496)
			st->ber_b = ((u32) dm.dmp_b_ber_err_bits * 100
			/ ((u32) dm.dmp_b_ber_rxd_bits / 100));
		else
			st->ber_b = ((u32) dm.dmp_b_ber_err_bits * 10000
			/ (u32) dm.dmp_b_ber_rxd_bits);
	} else {
		st->ber_b = 10000;
	}
	...
	if (dm.dmp_c_ber_rxd_bits) {
		if (dm.dmp_c_ber_err_bits > 429496)
			st->ber_c = ((u32) dm.dmp_c_ber_err_bits * 100
			/ ((u32) dm.dmp_c_ber_rxd_bits / 100));
		else
			st->ber_c = ((u32) dm.dmp_c_ber_err_bits * 10000
			/ (u32) dm.dmp_c_ber_rxd_bits);
	} else {
		st->ber_c = 10000;
	}
	...
	if (dm.vit_a_ber_rxd_rsps) {
		if (dm.vit_a_ber_err_bits & 0x00600000)
			rsber_A = ((u32)dm.vit_a_ber_err_bits * 250
			/ ((u32) dm.vit_a_ber_rxd_rsps * 204/5));
		else
			rsber_A = ((u32)dm.vit_a_ber_err_bits * 625
			/ ((u32) dm.vit_a_ber_rxd_rsps * 102));
	} else
		rsber_A =  10000;

	if (dm.vit_b_ber_rxd_rsps) {
		if (dm.vit_b_ber_err_bits & 0x00600000)
			rsber_B = ((u32) dm.vit_b_ber_err_bits * 250
			/ ((u32) dm.vit_b_ber_rxd_rsps * 204/5));
	else
			rsber_B = ((u32) dm.vit_b_ber_err_bits * 625
			/ ((u32) dm.vit_b_ber_rxd_rsps * 102));
	} else
		rsber_B =  10000;

	if (dm.vit_c_ber_rxd_rsps) {
		if (dm.vit_c_ber_err_bits & 0x00600000)
			rsber_C = ((u32) dm.vit_c_ber_err_bits * 250
			/ ((u32) dm.vit_c_ber_rxd_rsps * 204/5));
		else
			rsber_C = ((u32) dm.vit_c_ber_err_bits * 625
			/ ((u32) dm.vit_c_ber_rxd_rsps * 102));
	} else
		rsber_C = 10000;
	...
	if (!(tmcc_done & 0x01)) {
		...
		bbm_com_bulk_read(NULL, DIV_MASTER, 0x4110
			, &tmcc_data[0], 26);

		/*      */
		st->layerinfo_a = ((tmcc_mod_scheme((tmcc_data[3]
		& 0x70) >> 4)) & 0x07) << 13;
		st->layerinfo_a |= ((tmcc_code_rate(((tmcc_data[4] & 0x03) << 1)
			+ ((tmcc_data[3] & 0x80) >> 7))) & 0x07) << 10;
		st->layerinfo_a |= ((tmcc_inter_length(
			(tmcc_data[4] & 0x1c) >> 2, mode)) & 0x3f) << 4 ;
		st->layerinfo_a |= (tmcc_num_segment(((tmcc_data[5]
			& 0x01) << 3) + ((tmcc_data[4] & 0xe0) >> 5))) & 0x0f;


		/*      */
		st->layerinfo_b = ((tmcc_mod_scheme((tmcc_data[5]
		& 0x0e) >> 1)) & 0x07) << 13;
		st->layerinfo_b |= ((tmcc_code_rate((tmcc_data[5]
			& 0x70) >> 4)) & 0x07) << 10;
		st->layerinfo_b |= ((tmcc_inter_length(((tmcc_data[6]
			& 0x03) << 1) + ((tmcc_data[5] & 0x80) >> 7)
			, mode)) & 0x3f) << 4 ;
		st->layerinfo_b |= (tmcc_num_segment((tmcc_data[6]
			& 0x3c) >> 2)) & 0x0f;


		/*      */
		st->layerinfo_c = ((tmcc_mod_scheme(((tmcc_data[7]
		& 0x01) << 2) + ((tmcc_data[6] & 0xc0) >> 6))) & 0x07) << 13;
		st->layerinfo_c |= ((tmcc_code_rate((tmcc_data[7]
			& 0x0e) >> 1)) & 0x07) << 10;
		st->layerinfo_c |= ((tmcc_inter_length((tmcc_data[7]
			& 0x70) >> 4, mode)) & 0x3f) << 4 ;
		st->layerinfo_c |= (tmcc_num_segment(((tmcc_data[8]
			& 0x07) << 1) + ((tmcc_data[7] & 0x80) >> 7))) & 0x0f;
	}

	if ((st->layerinfo_a & 0x0f) == 1) {
		if (brd_type == ISDBT_13SEG) {
			if (rsber_A == 0) {
				st->ber_a = (st->ber_a > DC_OFFSET) ?
					(st->ber_a - DC_OFFSET) : (0);
			} else {
				st->ber_a = (st->ber_a
				> (DC_OFFSET - (((rsber_A * 2) > DC_OFFSET)
				? (DC_OFFSET) : (rsber_A * 2))))
				? (st->ber_a - (DC_OFFSET - (((rsber_A * 2)
				> DC_OFFSET) ? (DC_OFFSET) : (rsber_A * 2))))
					: (0);
			}
		}
		...
		if (brd_type == ISDBT_13SEG) {
			if (rsber_B == 0) {
				st->ber_b = (st->ber_b > DC_OFFSET)
					? (st->ber_b - DC_OFFSET) : (0);
			} else {
				st->ber_b = (st->ber_b > (DC_OFFSET
					- (((rsber_B * 2) > DC_OFFSET)
					? (DC_OFFSET) : (rsber_B * 2))))
		...
		if (brd_type == ISDBT_13SEG) {
			if (rsber_C == 0) {
				st->ber_c = (st->ber_c > DC_OFFSET)
					? (st->ber_c - DC_OFFSET) : (0);
			} else {
				st->ber_c = (st->ber_c
				> (DC_OFFSET - (((rsber_C * 2) > DC_OFFSET)
				? (DC_OFFSET) : (rsber_C * 2))))
				? (st->ber_c - (DC_OFFSET - (((rsber_C * 2)
				> DC_OFFSET) ? (DC_OFFSET) : (rsber_C * 2))))
				: (0);
			}
		}
	...
	transmission_parameter_switching = ((tmcc_data[2] & 0x80) >> 5)
						...
						+ ((tmcc_data[3] & 0x01) << 1);
	if (tmcc_done & 0x01) {
		st->tmccinfo = (tmcc_system_identification(
			(tmcc_data[2] & 0x30) >> 4) << 6)
	...
	st->antenna_level_oneseg = tunerbb_drv_fc8300_get_oneseg_antenna(
		st->antenna_level_oneseg, st->ber_1seg, st->cn, brd_type);
	st->antenna_level_fullseg = tunerbb_drv_fc8300_get_fullseg_antenna(
		st->antenna_level_fullseg, st->ber_fullseg, st->cn);
...
#ifdef BBM_EXT_LNA
{
    ...
    bbm_com_tuner_read(NULL, DIV_MASTER, 0xea, 1, &lna_control, 1);

    if (lna_control == 1)
        bbm_com_byte_write(NULL, BBM_GPIO_DATA, 0x00);
    else
        bbm_com_byte_write(NULL, BBM_GPIO_DATA, 0x04);
}
#endif
	print_log(NULL, "ant_oneseg : %d,  ber_oneseg : %d, \
		ant_fullseg : %d,  ber_fullseg : %d, switching : %d\n"
		, st->antenna_level_oneseg, st->ber_1seg
		, st->antenna_level_fullseg, st->ber_fullseg
		, st->fullseg_oneseg_flag);
	print_log(NULL, "LOCK : %d, CN : %d, RSSI : %d, msnr : %d\n"
		, st->lock, st->cn, st->rssi, cn*25);
	print_log(NULL, "BER_A : %d, BER_B : %d, PER_A : %d, PER_B : %d\n"
		, st->ber_a, st->ber_b, st->per_a, st->per_b);
	print_log(NULL, "layA : 0x%x, layB : 0x%x, layC : 0x%x,\
		TMCC : 0x%x , sysinfo : 0x%x \n", st->layerinfo_a
		, st->layerinfo_b, st->layerinfo_c, st->tmccinfo, st->sysinfo);
	return res;
}
